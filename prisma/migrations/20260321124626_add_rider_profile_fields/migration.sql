...
CREATE TABLE "restaurants" (
    ...
    "cuisine_types" TEXT[],
    "taguzair@uzair-HP-EliteBook-840-G7-Notebook-PC:~/food-delivery-backend$ npx prisma migrate dev --name add_rider_profile_fields
...
[*] Changed the `admin_logs` table
  [+] Added index on columns (admin_id)
  [+] Added index on columns (created_at)
  [+] Added index on columns (entity, entity_id)
  [+] Added foreign key on columns (admin_id)

[*] Changed the `cart_items` table
  [+] Added index on columns (user_id)
  [+] Added unique index on columns (user_id, menu_item_id)
  [+] Added foreign key on columns (menu_item_id)
...
[*] Changed the `coupon_usage` table
  [+] Added index on columns (coupon_id)
  [+] Added unique index on columns (coupon_id, order_id)
  [+] Added index on columns (user_id)
  [+] Added foreign key on columns (coupon_id)
  [+] Added foreign key on columns (user_id)

[*] Changed the `coupons` table
  [+] Added index on columns (code)
  [+] Added unique index on columns (code)
  [+] Added index on columns (is_active)
  [+] Added index on columns (valid_from, valid_until)

[*] Changed the `delivery_persons` table
  [+] Added unique index on columns (cnic_number)
  [+] Added index on columns (current_latitude, current_longitude)
  [+] Added index on columns (status)
  [+] Added unique index on columns (user_id)
  [+] Added foreign key on columns (user_id)

[*] Changed the `favorites` table
  [+] Added index on columns (restaurant_id)
  [+] Added index on columns (user_id)
  [+] Added unique index on columns (user_id, restaurant_id)
  [+] Added foreign key on columns (restaurant_id)
  [+] Added foreign key on columns (user_id)

[*] Changed the `menu_items` table
  [+] Added index on columns (category_id)
  [+] Added index on columns (is_available)
  [+] Added index on columns (price)
  [+] Added index on columns (restaurant_id)
  [+] Added unique index on columns (restaurant_id, slug)
  [+] Added index on columns (slug)
...
[*] Changed the `order_items` table
  [+] Added index on columns (order_id)
  [+] Added foreign key on columns (menu_item_id)
  [+] Added foreign key on columns (order_id)

[*] Changed the `orders` table
  [+] Added index on columns (created_at)
  [+] Added index on columns (customer_id)
  [+] Added index on columns (delivery_person_id)
  [+] Added index on columns (order_number)
  [+] Added unique index on columns (order_number)
  [+] Added index on columns (restaurant_id)
  [+] Added index on columns (status)
  [+] Added foreign key on columns (coupon_id)
  [+] Added foreign key on columns (customer_id)
  [+] Added foreign key on columns (delivery_address_id)
  [+] Added foreign key on columns (delivery_person_id)
  [+] Added foreign key on columns (restaurant_id)

[*] Changed the `payments` table
  [+] Added index on columns (cash_deposit_status)
  [+] Added unique index on columns (order_id)
  [+] Added index on columns (status)
  [+] Added index on columns (transaction_id)
  [+] Added unique index on columns (transaction_id)
  [+] Added foreign key on columns (order_id)

[*] Changed the `restaurants` table
  [+] Added index on columns (average_rating)
  [+] Added index on columns (is_featured)
  [+] Added index on columns (is_open)
  [+] Added index on columns (latitude, longitude)
...
[*] Changed the `reviews` table
  ...
  [+] Added unique index on columns (order_id)
  [+] Added index on columns (restaurant_id)
  [+] Added index on columns (restaurant_rating)
  [+] Added foreign key on columns (customer_id)
  [+] Added foreign key on columns (order_id)
  [+] Added foreign key on columns (restaurant_id)

[*] Changed the `rider_cash_deposits` table
  [+] Added index on columns (rider_id)
  [+] Added index on columns (status)
  [+] Added foreign key on columns (rider_id)

[*] Changed the `rider_documents` table
  [+] Added index on columns (delivery_person_id)
  [+] Added index on columns (status)
  [+] Added foreign key on columns (delivery_person_id)

[*] Changed the `user_addresses` table
  [+] Added index on columns (latitude, longitude)
  [+] Added index on columns (user_id)
  [+] Added foreign key on columns (user_id)
...
[*] Changed the `users` table
  ...
  [+] Added foreign key on columns (active_coupon_id)

? We need to reset the "public" schema at "aws-1-ap-southeast-2.pooler.supabase.com:5432"
Do you want to continue? All data will be lost. › (y/N)s" TEXT[],
    "created_at" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updated_at" TIMESTAMP(3) NOT NULL,
